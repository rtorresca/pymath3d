'''
This file is part of pymath3d.

pymath3d is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

pymath3d is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
GNU General Public License for more details.

You should have received a copy of the GNU General Public License
along with pymath3d.  If not, see <http://www.gnu.org/licenses/>.

Revision: $Rev$
ID: $Id$

Module implementing the Orientation class. The orientation is
represented internally by an orthogonal 3x3 matrix.
'''

from types import InstanceType
from math import pi,cos,sin
from copy import copy
import exceptions

from numpy import array,ndarray,transpose,cross,dot

from math3d.vector import Vector, isVector
import math3d.quaternion
from math3d.utils import isSequence, isNumTypes

## Possibly replace by 100*numpy.finfo(float).eps
_eps = 0.0000001

def isOrientation(o):
    #return (type(o) == InstanceType and o.__class__ == Orientation)
    return type(o) == Orientation

class Orientation(object):
    ''' An Orientation is a member of SO(3) which can be used either
    to perform a rotational transformation, or for keeping an
    orientation in 3D.'''
    
    class Error(exceptions.Exception):
        ''' Exception class.'''
        def __init__(self, message):
            self.message = message
        def __repr__(self):
            return self.__class__ + '.Error :' + self.message

    @classmethod
    def canCreateOn(cls, *args):
        """ Infer whether an Orientation can be constructed on the
        given argument tuple. This is mostly syntactic, down to the
        level of testing whether basic types are numeric. Testing
        whether constraints among the individual basic values are
        violated is not tested."""
        if len(args) == 1:
            arg = args[0]
            if type(arg) in [Orientation, math3d.quaternion.Quaternion]:
                return True
            elif isSequence(arg):
                return self.canCreateOn(*arg)
        elif len(args) == 3:
            for arg in args:
                if not Vector.canCreateOn(arg):
                    return False
                return True
        elif len(args) == 12:
            return isNumTypes(args)
        else:
            return False

    @classmethod
    def __createOnSequence(cls, args):
        """ Assuming args to be a sequence, try to create the data of
        an orientation over it."""
        if len(args) == 3:
            vs = [Vector(a) for a in args]
            return array([v._data for v in vs])
        elif len(args) == 12:
            oa=array(args)
            oa.shape=[3,3]
            return oa
        else:
            raise Orientation.Error('Could not create Orientation data on "%s"' % str(args))
                
    def __init__(self, *args):
        """ Create an orientation on another Orientation, a
        Quaternion, three Vectors, 12 numbers, or a 3x3 array. In case
        of 12 numbers, they are perceived as row-major order."""
        if len(args) == 1:
            arg=args[0]
            if isOrientation(arg):
                self._data = arg._data.copy()
            elif math3d.quaternion.isQuaternion(arg):
                self._data = arg.toOrientation()._data.copy()
            elif isSequence(arg):
                self._data = Orientation.__createOnSequence(arg)
        elif len(args) > 1:
            self._data = Orientation.__createOnSequence(args)
        elif len(args) == 0:
            self._data = array([Vector.e0._data, Vector.e1._data, Vector.e2._data])

    def __copy__(self):
        """Copy method for creating a copy of this Orientation."""
        o = Orientation()
        o.copy(self)
        return o
    
    def __deepcopy__(self, memo):
        return self.__copy__()
    
    def copy(self, other):
        """Copy data from other to self. """
        self._data = array(map(copy, other._data))
    
    def __getattr__(self, name):
        if name == 'data':
            return self._data
        elif name == 'colX':
            return transpose(self._data)[0]
        elif name == 'colY':
            return transpose(self._data)[1]
        elif name == 'colZ':
            return transpose(self._data)[2]
        elif name == 'vecX':
            return Vector(transpose(self._data)[0])
        elif name == 'vecY':
            return Vector(transpose(self._data)[1])
        elif name == 'vecZ':
            return Vector(transpose(self._data)[2])
        else:
            raise self.Error, 'Orientation does not have attribute "%s"' % name
        
    def __getitem__(self, indices):
        return self._data.__getitem__(indices)
    
    def __coerce__(self, other):
        return None
        
    def __setattr__(self, name, val):
        if name == '_data':
            self.__dict__['_data']=val
        elif name[:3] in ['vec', 'col'] and name[3] in 'XYZ':
            ## This is dangerous since there is no automatic
            ## re-normalization
            _td=transpose(self._data)
            if isVector(val):
                val=val._data
            if name == 'colX' or name == 'vecX':
                _td[0]=val
            elif name == 'colY' or name == 'vecY':
                _td[1]=val
            elif name == 'colZ' or name == 'vecZ':
                _td[2]=val
            self._data=transpose(_td)

        
    def fromXY(self, cx, cy):
        ''' Reset this orientation to the one that conforms with the
        given x and y directions.'''
        if cx * cy > _eps:
            print 'warning ... orthogonalizing!'
        self.colX = cx.normalized()
        self.colY = cy.normalized()
        self.colZ = cx.cross(cy).normalized()
        ## A last normalization check!
        #if self.colX.dist(self.colY.cross(self.colZ)) > _eps:
        self.colX=cross(self.colY,self.colZ)
        
    def fromXZ(self, cx, cz):
        ''' Reset this orientation to the one that conforms with the
        given x and z directions.'''
        if cx * cz > _eps:
            print 'warning ... orthogonalizing!'
        self.colX = cx.normalized()
        self.colZ = cz.normalized()
        self.colY = cz.cross(cx).normalized()
        ## A last normalization check!
        #if self.colX.dist(self.colY.cross(self.colZ)) > _eps:
        self.colX = cross(self.colY, self.colZ)

    def toAxisAngle(self):
        """ Return a pair (axis,angle) representing the equivalent
        orientation."""
        q = math3d.quaternion.Quaternion(self)
        return q.toAxisAngle()
        
    def fromAxisAngle(self, axis, angle):
        """ Set this orientation to the equivalent to rotation of
        'angle' around 'axis'."""
        if not isVector(axis):
            axis = Vector(axis)
        axis.normalize()
        x = axis._data[0]
        y = axis._data[1]
        z = axis._data[2]
        ct =cos(angle)
        st = sin(angle)
        self._data = array([ \
            [ct + (1 - ct) * x**2,
             (1 - ct) * x * y - st *z,
             (1 - ct) * x * z + st * y],
            [(1 - ct) * x * y + st * z,
             ct + (1 - ct) * y**2,
             (1 - ct) * y * z - st * x],
            [(1 - ct) * x * z - st * y,
             (1 - ct) * y * z + st * x,
             ct + (1 - ct) * z**2]])

    def rotX(self, angle):
        """ Replace this orientation by that of a rotation around x."""
        ca = cos(angle)
        sa = sin(angle)
        self._data = array([[1, 0, 0], [0, ca, -sa], [0, sa, ca]])
        #self.fromAxisAngle(Vector.e0, angle)
        
    def rotY(self, angle):
        """ Replace this orientation by that of a rotation around y."""
        ca=cos(angle)
        sa=sin(angle)
        self._data = array([[ca, 0, sa], [0, 1, 0], [-sa, 0, ca]])
        #self.fromAxisAngle(Vector.e1, angle)
        
    def rotZ(self, angle):
        """ Replace this orientation by that of a rotation around z. """
        ca = cos(angle)
        sa=sin(angle)
        self._data = array([[ca, -sa, 0], [sa, ca, 0], [0, 0, 1]])
        #self.fromAxisAngle(Vector.e2,angle)

    def rotateT(self, axis, angle):
        """ In-place rotation of this orientation angle radians in
        axis perceived in the transformed reference system."""
        o = Orientation()
        o.fromAxisAngle(axis, angle)
        self.copy(self * o)
    rotate = rotateT
    
    def rotateB(self, axis, angle):
        """ In-place rotation of this orientation angle radians in
        axis perceived in the base reference system."""
        o = Orientation()
        o.fromAxisAngle(axis, angle)
        self.copy(o * self)

    def rotateXB(self, angle):
        """ In-place rotation of this oriantation by a rotation around
        x axis in the base reference system. (Inefficient!)"""
        self.rotateB(Vector.e0, angle)
    
    def rotateYB(self, angle):
        """ In-place rotation of this oriantation by a rotation around
        y axis in the base reference system. (Inefficient!)"""
        self.rotateB(Vector.e1, angle)
    
    def rotateZB(self, angle):
        """ In-place rotation of this oriantation by a rotation around
        z axis in the base reference system. (Inefficient!)"""
        self.rotateB(Vector.e2, angle)
    
    def rotateXT(self, angle):
        """ In-place rotation of this oriantation by a rotation around
        x axis in the transformed reference system. (Inefficient!)"""
        self.rotateT(Vector.e0, angle)
    rotateX = rotateXT
    
    def rotateYT(self,angle):
        """ In-place rotation of this oriantation by a rotation around
        y axis in the transformed reference system. (Inefficient!)"""
        self.rotateT(Vector.e1,angle)
    rotateY = rotateYT
    
    def rotateZT(self, angle):
        """ In-place rotation of this oriantation by a rotation around
        z axis in the transformed reference system. (Inefficient!)"""
        self.rotateT(Vector.e2, angle)
    rotateZ = rotateZT
    
    def __repr__(self):
        return '<Orientation: \n' + repr(self._data) + '>'

    def __str__(self):
        return self.__repr__()
    
    def invert(self):
        """ In-place inversion of this orientation."""
        self._data = transpose(self._data)

    def inverse(self):
        """ Return an inverse of this orientation as a rotation."""
        o = Orientation(self._data)
        o.invert()
        return o

    def __mul__(self, other):
        if isOrientation(other):
            return Orientation(dot(self._data, other.data))
        elif isVector(other):
            return Vector(dot(self._data, other.data))
        elif isSequence(other):
            return map(self.__mul__, other)
        
def newOrientFromXY(cx, cy):
    ''' Create an orientation conforming with the given x and y
    directions.'''
    o = Orientation()
    o.fromXY(cx, cy)
    return o

def newOrientFromXZ(cx, cz):
    ''' Create an orientation conforming with the given x and z
    directions.'''    
    o = Orientation()
    o.fromXZ(cx, cz)
    return o

def newOrientRotZ(angle):
    ''' Create an orientation corresponding to a rotation for "angle"
    around the z direction.'''
    o = Orientation()
    o.rotZ(angle)
    return o

def newOrientRotX(angle):
    ''' Create an orientation corresponding to a rotation for "angle"
    around the x direction.'''
    o = Orientation()
    o.rotX(angle)
    return o

def newOrientRotY(angle):
    ''' Create an orientation corresponding to a rotation for "angle"
    around the y direction.'''
    o = Orientation()
    o.rotY(angle)
    return o

if __name__ == '__main__':
    o = Orientation()
    r = Orientation()
    o.fromXY(Vector(1, 1, 0), Vector(-1, 1, 0))
    r.rotZ(pi / 2)
    ro = r * o
