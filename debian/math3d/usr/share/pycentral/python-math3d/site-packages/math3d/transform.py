...
class Transform(object):
    ...
    def __init__(self, *args):
        if len(args) == 0 or len(args) == 1 and \
            ...
            if type(arg) == Transform or \
                ...
                self._data=arg
                self._o = Orientation(self._data[:3,:3])
                self._v = Vector(self._data[:3,3:4].reshape(3))
            else:
                raise self.Error, \
                      "Could not create Transform on arguments : " + str(args)                
        elif len(args) >= 1:
    ...
    def _setFromOV(self, o, v):
        ...
        self._data[:3,3:4] = v._data.reshape(3,1)
        
    def __getattr__(self, name):
        if name == 'orient':
            #return self._o
            return Orientation(self._data[:3,:3])
        elif name == 'pos':
            #return self._v
            return Vector(self._data[:3,3:4].reshape(3))
        else:
    ...
    def copy(self, other):
        ...
        self._setFromOV(other._o, other._v)
    ...
    def toArray(self):
        ...
        return (self._data[:3,:3], self._data[:3,3:4].reshape(3))
    
    def toList(self):
        ''' Return a list with orientation and position in list form.'''
        #return [self._o._data.tolist(),self._v._data.tolist()]
        return [self._data[:3,:3].tolist(), self._data[:3,3:4].reshape(3).tolist()]
