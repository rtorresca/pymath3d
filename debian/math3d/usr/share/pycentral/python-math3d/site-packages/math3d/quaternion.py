...
class Quaternion(object):
    ...
    def __init__(self, *args):
        ...
        if len(args) == 0:
            ...
            if math3d.orientation.isOrientation(args[0]):
                self._v = Vector()
                self.fromOrientation(args[0])
            ## Copy constructor
            elif isQuaternion(args[0]):
                self._s = args[0]._s
                self._v = copy(args[0]._v)
        elif len(args) == 2:
    ...
    def copy(self):
        ''' Return a quaternion which is a copy of this quaternion. '''
        return Quaternion(self._s, self._v)
    ...
    def dist(self, other):
        ...
        return sqrt((self._s - other._s)**2 + (self._v - other._v).length2())
        
    def toAxisAngle(self):
        ''' Return an angle and axis pair representing the orientation
        of this quaternion.'''
        theta = 2 * acos(self._s)
        if theta != 0:
            w = self._v / sin(theta / 2)
        else:
            w = Vector()
        return (w, theta)
    ...
    def toOrientation(self):
        ...
        y2 = y**2
        z2 = z**2
        return math3d.orientation.Orientation(array([
