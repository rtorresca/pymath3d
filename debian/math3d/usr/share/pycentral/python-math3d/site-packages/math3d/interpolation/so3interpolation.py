...
Module implementing the SO3 interpolator class.
'''
...
class SO3Interpolation(object):
    ...
    def __init__(self, start, end, shortest=True):
        ...
        self._qstart = Quaternion(start) if isOrientation(start) else start
        self._qend = Quaternion(end) if isOrientation(end) else end
    ...
    def quat(self, time, checkrange=True):
        ...
        if checkrange:
            ...
            if time < 0.0 or time > 1.0:
                raise self.Error, '"time" must be number in [0,1]. Was ', time 
    ...
    def orient(self, time, checkrange=True):
        ...
        if checkrange:
            ...
            if time < 0.0 or time > 1.0:
                raise self.Error, '"time" must be number in [0,1]. Was ', time 
        return self.quat(time, False).toOrientation()
