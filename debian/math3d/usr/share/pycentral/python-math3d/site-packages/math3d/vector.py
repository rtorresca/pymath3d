...
class Vector(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 3 and isNumTypes(args):
            self._data=numpy.array(map(float, args))
        elif len(args) == 2 and isNumTypes(args):
            self._data = numpy.array(map(float,[args[0], args[1], 0]))
        elif len(args) == 1:
            arg = args[0]
            if isThreeSequence(arg):
                self._data = numpy.array(map(float, arg))
            elif isSequence(arg) and len(arg)  == 2:
                self._data = numpy.array(map(float, [arg[0], arg[1], 0]))
    ...
    def __eq__(self,other):
        if isVector(other) and (self._data==other._data).all():
            return True
        else:
            return False
