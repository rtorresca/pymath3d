...
class Vector(object):
    ...
    def canCreateOn(cls, arg):
        if type(arg) == cls:
            return True
        elif isSequence(arg) and len(arg) == 3 and isNumTypes(arg):
            return True
        else:
            return False
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 3 and isNumTypes(args):
            ...
            if isThreeSequence(args[0]):
                self._data = array(map(float, args[0]))
            elif isSequence(args[0]) and len(args[0])  == 2:
                self._data = array(map(float, [args[0][0], args[0][1], 0]))
            elif isVector(args[0]):
